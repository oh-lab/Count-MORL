...
from hashcount import HashCount
...
class BNN:
    ...
    def __init__(self, params):
        ...
        self.bit = params['bit']
        self.cnt_hidden_dim = params['cnt_hidden_dim']
        self.hash_set = {}
        ...
        self.decays_cnt, self.optvars_cnt = [], []

        self.end_act, self.end_act_name = None, None
        self.scaler = None

        # Training objects
        self.optimizer, self.optimizer_cnt = None, None
        self.sy_train_in, self.sy_train_targ, self.sy_train_in_cnt, self.sy_train_targ_cnt = None, None, None, None
        self.train_op, self.mse_loss = None, None
        self.train_op_cnt, self.mse_loss_cnt = None, None

        # Prediction objects
        self.sy_pred_in2d, self.sy_pred_mean2d_fac, self.sy_pred_var2d_fac, self.sy_pred_latent2d_fac = None, None, None, None
        self.sy_pred_mean2d, self.sy_pred_var2d = None, None
        self.sy_pred_in3d, self.sy_pred_mean3d_fac, self.sy_pred_var3d_fac, self.sy_pred_latent3d_fac = None, None, None, None
        ...
        self.use_count = params.get('use_count', False)
        if self.use_count:
            self.cnt_layers = []
        else:
            self.cnt_layers = None

        for ind in range(self.num_nets):
            hash_ftn = HashCount(dim_key = self.bit, obs_dim = self.bit)
            self.hash_set[ind] = hash_ftn
        print('[ BNN ] Single Hashcount function, {} bit'.format(self.bit))
    ...
    def add(self, layer, var_layer=False, cnt_layer=False):
        ...
        if not var_layer and not cnt_layer:
            ...
            if len(self.cnt_layers) > 0:
                layer.set_input_dim(self.cnt_layers[-1].get_output_dim())
            self.cnt_layers.append(layer.copy())
        
        print('Added layer with input dim', layer.get_input_dim(), ', output dim', layer.get_output_dim())

    def pop(self, var_layer=False, cnt_layer=False):
    ...
    def finalize(self, optimizer, optimizer_args=None, *args, **kwargs):
        ...
        Returns: None
        """
        if len(self.layers) == 0 or (self.separate_mean_var and len(self.var_layers) == 0) or (self.use_count and len(self.cnt_layers) == 0):
        ...
        self.optimizer_cnt = optimizer(**optimizer_args)
                ...
                with tf.variable_scope(self.name):
                    ...
                    for i, layer in enumerate(self.var_layers):
                        with tf.variable_scope("Layer%i_var" % i):
                            ...
                            self.decays.extend(layer.get_decays())
                            self.optvars.extend(layer.get_vars())
                    for i, layer in enumerate(self.cnt_layers):
                        with tf.variable_scope("Layer%i_cnt" % i):
                            layer.construct_vars()
                            self.decays_cnt.extend(layer.get_decays())
                            self.optvars_cnt.extend(layer.get_vars())
        ...
        with tf.variable_scope(self.name):
            ...
            if not self.separate_mean_var:
                ...
                self.sy_train_targ = tf.placeholder(dtype=tf.float32,
                                                    ...
                                                    name="training_targets")
            if self.use_count:
                self.sy_train_targ_cnt = tf.placeholder(dtype=tf.float32,
                                                        shape=[self.num_nets, None, self.cnt_layers[-1].get_output_dim()],
                                                        name="training_targets_cnt")
            if not self.deterministic:
                ...
                if self.use_count:
                    train_loss_cnt = self._compile_losses_cnt(self.sy_train_in, self.sy_train_targ_cnt)
                    train_loss_cnt += tf.add_n(self.decays_cnt)
            else:
            ...
            self.mse_loss_cnt = self._compile_losses_cnt(self.sy_train_in, self.sy_train_targ_cnt)
            self.train_op_cnt = self.optimizer_cnt.minimize(train_loss_cnt, var_list=self.optvars_cnt)

        # Initialize all variables
        self.sess.run(tf.variables_initializer(self.optvars + self.nonoptvars + self.optimizer.variables()))
        self.sess.run(tf.variables_initializer(self.optvars_cnt + self.optimizer_cnt.variables()))
        ...
        with tf.variable_scope(self.name):
            ...
            self.sy_pred_latent2d_fac =  \
                self.create_prediction_tensors_latent(self.sy_pred_in2d, factored=True)

            self.sy_pred_latent2d = tf.reduce_mean(self.sy_pred_latent2d_fac, axis=0)
            ...
            self.sy_pred_latent3d_fac = \
                self.create_prediction_tensors_latent(self.sy_pred_in3d, factored=True)
    ...
    def load_params(self):
        with self.sess.as_default():
            ...
            all_vars = self.nonoptvars + self.optvars + self.optvars_cnt
    ...
    def _save_state(self, idx):
        ...
        if self.use_count:
            self._state[idx].extend([layer.get_model_vars(idx, self.sess) for layer in self.cnt_layers])
    ...
    def _end_train_cnt(self, holdout_losses_cnt):
        sorted_inds = np.argsort(holdout_losses_cnt)
        self._model_inds_cnt = sorted_inds[:self.num_elites].tolist()
        print('Using {} / {} cnt models: {}'.format(self.num_elites, self.num_nets, self._model_inds_cnt))
    ...
    def train(self, inputs, targets,
        ...
        for epoch in epoch_iter:
            for batch_num in range(int(np.ceil(idxs.shape[-1] / batch_size))):
                ...
                self.sess.run(
                    self.train_op_cnt,
                    feed_dict={self.sy_train_in: inputs[batch_idxs], self.sy_train_targ_cnt: inputs[batch_idxs]}
                )
                grad_updates += 1
            ...
            if not hide_progress:
                if holdout_ratio < 1e-12:
                    ...
                    losses_cnt = self.sess.run(
                            self.mse_loss_cnt,
                            feed_dict={
                                self.sy_train_in: inputs[idxs[:, :max_logging]],
                                self.sy_train_targ_cnt: inputs[idxs[:, :max_logging]]
                            }
                        )
                    named_losses = [['M{}'.format(i), losses[i]] for i in range(len(losses))]
                    named_losses_cnt = [['MC{}'.format(i), losses[i]] for i in range(len(losses_cnt))]
                    named_losses_total = named_losses + named_losses_cnt
                    progress.set_description(named_losses_total)
                else:
                    losses = self.sess.run(
                            self.mse_loss,
                            feed_dict={
                    ...
                    holdout_losses_cnt = self.sess.run(
                            self.mse_loss_cnt,
                            feed_dict={
                                self.sy_train_in: holdout_inputs,
                                self.sy_train_targ_cnt: holdout_inputs
                            }
                        )

                    named_losses = [['M{}'.format(i), losses[i]] for i in range(len(losses))]
                    named_holdout_losses = [['V{}'.format(i), holdout_losses[i]] for i in range(len(holdout_losses))]
                    named_losses_cnt = [['MC{}'.format(i), losses_cnt[i]] for i in range(len(losses_cnt))]
                    named_holdout_losses_cnt = [['VC{}'.format(i), holdout_losses_cnt[i]] for i in range(len(holdout_losses_cnt))]
                    named_losses_total = named_losses + named_holdout_losses + named_losses_cnt + named_holdout_losses_cnt + [['T', time.time() - t0]]
                    progress.set_description(named_losses_total)
        ...
        self._end_train_cnt(holdout_losses_cnt)
    ...
    def train_cnt(self, batch_size=1, *args, **kwargs):

        for batch_num in range(int(np.ceil(self.cnt_inputs.shape[-2] / batch_size))):
            hash_inputs_batch = self.cnt_inputs[batch_num * batch_size:(batch_num + 1) * batch_size, :]
            hash_inputs = self.predict_latent(hash_inputs_batch, factored=True)
            #print(hash_inputs.shape)
            #assert 1 == 2
            for ind, hash_ftn in self.hash_set.items():
                hash_ftn.compute_intrinsic_reward(hash_inputs[ind], train = True)
        print('[ BNN ] Hashcount | bit dim : {} | obs dim : {}'.format(self.bit, hash_inputs.shape[-1]))
        for idx in self._model_inds_cnt:
            num_cluster = self.hash_set[idx].compute_cluster()
            print('Using {} clusters in {} cnt model'.format(num_cluster, idx))

    def cnt(self, inputs, train=False, cnt_type='mean', cnt_coeff = 0.5, *args, **kwargs):
        
        temp = False
        for idx in self._model_inds_cnt:
            if temp:
                penalty_cnt = np.append(penalty_cnt, self.hash_set[idx].compute_intrinsic_reward(inputs[idx], train = False), axis = 1)
            else:
                penalty_cnt = self.hash_set[idx].compute_intrinsic_reward(inputs[idx], train = False)
                temp = True

        if cnt_type == "AVG":
            penalty_cnt = np.mean(penalty_cnt, axis = 1)
            penalty_cnt = 1 / np.sqrt(penalty_cnt)
            return np.expand_dims(penalty_cnt, 1)
        
        elif cnt_type == "UC":
            penalty_cnt = np.mean(penalty_cnt, axis=1) + cnt_coeff * np.std(penalty_cnt, axis=1)
            penalty_cnt = 1 / np.sqrt(penalty_cnt)
            return np.expand_dims(penalty_cnt, 1)

        elif cnt_type == "LC":
            penalty_cnt = np.mean(penalty_cnt, axis=1) - cnt_coeff * np.std(penalty_cnt, axis=1)
            penalty_cnt = 1 / np.sqrt(np.where(penalty_cnt>0, penalty_cnt, 1))
            return np.expand_dims(penalty_cnt, 1)
    ...
    def predict_latent(self, inputs, factored=False, *args, **kwargs):
    ...
    def create_prediction_tensors_latent(self, inputs, factored=False, *args, **kwargs):
        factored_latent = self._compile_outputs_latent(inputs)
        if inputs.shape.ndims == 2 and not factored:
            latent = tf.reduce_mean(factored_latent, axis=0)
            return latent
        return factored_latent
    ...
    def _compile_outputs_stac(self, inputs):
        """
        Arguments:
            inputs: (tf.Tensor) A tensor representing the inputs to the network

        Returns: (tf.Tensors) state-action predictions at inputs for each network
            in the ensemble.
        """
        cur_out = self.scaler.transform(inputs)
        mean = cur_out
        ...
        stac = mean
        for layer in self.cnt_layers:
            stac = layer.compute_output_tensor(stac)

        return stac

    def _compile_outputs_latent(self, inputs, ret_log_var=False):
        ...
        latent = mean
        for i, layer in enumerate(self.cnt_layers):
            latent = layer.compute_output_tensor(latent)
            if i == 2:
                break
    ...
    def _compile_losses_cnt(self, inputs, targets):
        stac = self._compile_outputs_stac(inputs)
        total_losses = tf.reduce_mean(tf.reduce_mean(tf.square(stac - targets), axis=-1), axis=-1)

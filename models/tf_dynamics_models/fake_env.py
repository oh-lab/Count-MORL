...
class FakeEnv:
    ...
    def step(self, obs, act, deterministic=False):
        ...
        if self.penalty_coeff != 0:
            if self.use_count:
                ensemble_model_hash = self.model.predict_latent(inputs, factored=True)
                penalty = self.model.cnt(ensemble_model_hash, cnt_type = self.cnt_type, cnt_coef = self.cnt_coef)
            else:
